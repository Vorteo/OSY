    ...
    while (1)
    {
        ...
        while (1) // wait for new client
        {
            ...
            if (l_read_poll[1].revents & POLLIN)
            {
                if (child == 0)
                {
                    printf("Dite nemuze pridavat klienty.\n");
                    break;
                }
                ...
                if (child == 0)
                {
                    ...
                    // Each child accepts a socket and continues to do work outside
                    if (l_sock_client == -1)
                    {
            ...
            if (l_read_poll[1].revents & POLLIN)
            {
                ...
                for (int i = 0; i < l_len; i++)
                {
                    ...
                    if (l_buf[i] == '\n' && i != l_len - 2)
                    {
                        ...
                        // End of general row
                        row_buffer = "[" + std::to_string(row_number++) + "] " + row_buffer + "\0";
                        int len = write(l_sock_client, row_buffer.c_str(), row_buffer.size() + 1); // .size() i strlen nevraci \0
