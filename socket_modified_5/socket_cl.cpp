...
int main(int t_narg, char **t_args)
{
    ...
    FILE *fd = fopen("socket_srv.cpp", "r");

    // struct stat st;
    // stat("zadani.txt", &st);
    // long int seek_size = st.st_size;

    fseek(fd, 0, SEEK_END);
    long int seek_size = ftell(fd);
    ...
    int len = write(l_sock_server, buf, strlen(buf) + 1);
    ...
    while (1)
    {
        ...
        if (l_read_poll[1].revents & POLLIN)
        {
            ...
            // Potvrzeno
            if (!strncasecmp(l_buf, "Potvrzuji", strlen("Potvrzuji")))
            {
                l_len = write(STDOUT_FILENO, "Potvrzeno od serveru, posilam data...\n", strlen("Potvrzeno od serveru, posilam data...\n"));

                if (fork() == 0)
                {
                    dup2(l_sock_server, STDOUT_FILENO);
                    execlp("cat", "cat", "socket_srv.cpp", nullptr);
                    printf("Tady bych se nemel dostat\n");
                    exit(-1);
                }
            }
            else
