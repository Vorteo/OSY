    ...
    while (1)
    {
        ...
        while (1) // wait for new client
        {
            ...
            { // new client?
                ...
                // child for new connection
                pid_t pid_client = fork();

                // child part, client code
                if (pid_client == 0)
                {
                    if (l_sock_client == -1)
                    {
                    ...
                    { // communication
                        ...
                        if (l_read_poll[1].revents & POLLIN)
                        {
                            ...
                            write(l_sock_client, head, sizeof(head));
                            write(l_sock_client, html_head, sizeof(html_head));
                            write(l_sock_client, html_body, sizeof(html_body));
                            write(l_sock_client, html_heel, sizeof(html_heel));

                            //-----------------
                            char *arguments_list[] = {"date", NULL};
                            int fd[2];
                            pipe(fd);
                            ...
                            {
                                close(fd[0]);
                                dup2(l_sock_client, 1);

                                execvp("date", arguments_list);
